defmodule DayFourSolution do
  ...
  def solve(input) do
    String.split(input, "\n")
    |> Enum.map(&is_room_valid/1)
    |> Enum.reduce(0, fn({room_validity, sector_id}, acc) ->
      case room_validity do
        :real ->
          acc + sector_id
        :decoy ->
          acc
  ...
  def is_room_valid(room) do
    [letters_with_dashes, checksum_with_closing_bracket] = String.split(room, "[", parts: 2)
    [given_checksum] = String.split(checksum_with_closing_bracket, "]", trim: true)
    parts = String.split(letters_with_dashes, "-")

    [sector_id] = Enum.take(parts, -1)
    frequency = String.slice(letters_with_dashes, 0, String.length(letters_with_dashes) - String.length(sector_id))
    |> String.replace("-", "")
    |> String.split("", trim: true)
    |> Enum.reduce(%{}, &update_count/2)

    [a, b, c, d, e] = Map.keys(frequency)
    |> Enum.sort(&(
      (
        Map.get(frequency, &1) > Map.get(frequency, &2)
      ) or (Map.get(frequency, &1) == Map.get(frequency, &2) and &1 < &2)
      ))
    |> Enum.take(5)
    computed_checksum = "#{a}#{b}#{c}#{d}#{e}"

    case computed_checksum do
      ^given_checksum ->
        real_room_name = decode_room_name(letters_with_dashes, String.to_integer(sector_id))
        IO.inspect(real_room_name)
        {:real, String.to_integer(sector_id)}
      _ ->
        {:decoy, String.to_integer(sector_id)}
    end
  end
  ...
  def decode_room_name(room_name, shift) do
    String.replace(room_name, "-", " ")
    |> String.to_charlist()
    |> Enum.map(fn(letter) -> decode_letter(letter, shift) end)
  end

  def decode_letter(letter, shift) when letter >= @first_letter and letter <= @last_letter do
    normalized = letter - @first_letter
    rem(normalized + shift, @last_letter - @first_letter + 1) + @first_letter
  end

  def decode_letter(letter, _) do
    letter
  end
end
...
IO.inspect(DayFourSolution.solve(input))
