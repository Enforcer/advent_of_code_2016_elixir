defmodule DaySevenSolution do
 defp is_abba(slice) do
    first_half = String.slice(slice, 0, 2)
    second_half = String.slice(slice, 2, 2)
    first_half == String.reverse(second_half) and first_half != second_half
  end

  defp is_aba(slice) do
    String.at(slice, 0) == String.at(slice, 2) and String.at(slice, 0) != String.at(slice, 1)
  end

  defp in_any_of_ranges(index, ranges) do
    Enum.reduce_while(ranges, false, fn(range, _acc) ->
      case index in range do
        true ->
          {:halt, true}
        false ->
          {:cont, false}
  ...
  defp find_counterpart_in_any_of_ranges(slice, ranges, address) do
    counterpart = String.at(slice, 1) <>String.at(slice, 0) <> String.at(slice, 1)

    Enum.reduce_while(ranges, false, fn(range, _acc) ->
      start..stop = range
      sliced_range = String.slice(address, start + 1, stop - start - 1)

      case :binary.match(sliced_range, counterpart) do
        :nomatch ->
          {:cont, false}
        {_index, _length} ->
          {:halt, true}
  ...
  def is_ssl_supported_for_address(address) do
    graphemes = String.graphemes(address)
    hypernet_ranges = get_hypernet_ranges(graphemes)

    get_sliced(graphemes, address, 3, fn(x) -> is_aba(x) end)
    |> Enum.reduce_while(false, fn({stop_position, is_aba}, acc) ->
      in_any_of_hypernet_ranges = in_any_of_ranges(stop_position, hypernet_ranges)

      case is_aba and not in_any_of_hypernet_ranges do
        true ->
          slice = String.slice(address, stop_position - 3 + 1, 3)
          has_bab = find_counterpart_in_any_of_ranges(slice, hypernet_ranges, address)
          case has_bab do
            true ->
              {:halt, true}
            false ->
  ...
  defp get_hypernet_ranges(graphemes) do
    graphemes
    |> Enum.with_index() |> Enum.reduce([], fn({letter, index}, acc) ->
      case letter do
        "[" ->
          [{index, nil} | acc]
        "]" ->
  ...
  defp get_sliced(graphemes, address, length, fun) do
    graphemes
    |> Enum.with_index()
    |> Enum.take(-String.length(address) + length - 1)
    |> Enum.map(fn({_grapheme, position}) ->
        slice = String.slice(address, position - length + 1, length)
        {position, fun.(slice)}
    end)
  end

  def is_tls_supported_for_address(address) do
    graphemes = String.graphemes(address)
    hypernet_ranges = get_hypernet_ranges(graphemes)

    get_sliced(graphemes, address, 4, fn(x) -> is_abba(x) end)
    |> Enum.reduce_while(false, fn({stop_position, is_abba}, acc) ->
      in_any_of_hypernet_ranges = in_any_of_ranges(stop_position, hypernet_ranges)

      case is_abba and in_any_of_hypernet_ranges do
        true ->
          {:halt, false}
        false ->
          {:cont, acc or is_abba}
  ...
  def solve(input) do
    String.split(input, "\n")
    |> Enum.map(&(is_tls_supported_for_address(&1)))
    |> Enum.count(&(&1))
  end

  def solve_part_two(input) do
    String.split(input, "\n")
    |> Enum.map(&(is_ssl_supported_for_address(&1)))
    |> Enum.count(&(&1))
  end
end
...
# IO.inspect(DaySevenSolution.solve(test_input))

test_input_second = "aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb"

IO.inspect(DaySevenSolution.solve_part_two(test_input_second))
...
IO.inspect(DaySevenSolution.solve(input))
IO.inspect(DaySevenSolution.solve_part_two(input))
